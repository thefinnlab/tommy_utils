...
#### PLOT THE INTERSECTION BETWEEN A CONTRAST AND IT'S DERIVATIVE #####
from nilearn.input_data import NiftiMasker
import dataset_utils as utils
from regression_utils import threshold_contrast

def plot_contrast_derivative_intersection(dataset_meta, task, sub, src_dtype, analysis, contrast_name, alpha=0.05, surf_type='fslr', views=['lateral', 'medial'], add_depth=True, colorbar=True, title=None, out_fn=None):
	
	masker = NiftiMasker(dataset_meta['mask']).fit()
	
	# def plot_group_contrast(dataset_meta, task, analysis, contrast_name, use_derivative=)
	contrast_map = utils.load_subject_data(dataset_meta, task=task, sub_list=sub, 
											 dtype=[src_dtype, analysis, 'contrasts', f'contrast-{contrast_name}'], load_option='load_nii')
	
	derivative_map = utils.load_subject_data(dataset_meta, task=task, sub_list=sub, 
											 dtype=[src_dtype, analysis, 'contrasts', f'contrast-{contrast_name}-derivative'], load_option='load_nii')
	
	# threshold the maps using zscores
	thresholded_map = threshold_contrast(contrast_map, masker, alpha=alpha)
	thresholded_derivative_map = threshold_contrast(derivative_map, masker, alpha=alpha)
	
	# transform the two together then inverse transform back
	dss = [masker.transform(ds).astype(bool) for ds in [thresholded_map, thresholded_derivative_map]]
	intersection_map = masker.inverse_transform(np.logical_and(*dss))
	
	# map to the surface
	surfs, ds_contrast = vol_to_surf(thresholded_map, surf_type=surf_type)
	_, ds_derivative = vol_to_surf(thresholded_derivative_map, surf_type=surf_type)
	_, ds_intersection = vol_to_surf(intersection_map, surf_type=surf_type)
	
	cmap_contrast = ListedColormap(['yellow'], 'contrast', N=1)
	cmap_derivative = ListedColormap(['dodgerblue'], 'derivative', N=1)
	cmap_intersection = ListedColormap(['purple'], 'intersection', N=1)
									 
	contrast = make_layers_dict(data=ds_contrast, cmap=cmap_contrast, label=contrast_name.capitalize())
	derivative = make_layers_dict(data=ds_derivative, cmap=cmap_derivative, label='Derivative')
	intersection = make_layers_dict(data=ds_intersection, cmap=cmap_intersection, label='Intersection')
	
	layers_info = [contrast, derivative, intersection]
	fig, p = plot_surf_data(surfs, layers_info, surf_type=surf_type, views=views, embed_nb=True, colorbar=colorbar, add_depth=add_depth, brightness=0.5, cbar_loc='right')
